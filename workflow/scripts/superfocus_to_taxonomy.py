...
def parse_m8_tophit(m8f, evalue, database=None, verbose=False):
    """
    Parse the m8 file and ... do something!
    :param m8f: the m8 output file from diamond
    :param evalue: the maximum evalue
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--db', help='SQLite3 taxonomy database location')
    parser.add_argument('-v', help='verbose output', action='store_true')
    args = parser.parse_args()


    if args.tophit:
        parse_m8_tophit(args.file, args.evalue, args.db, args.v)
    else:
