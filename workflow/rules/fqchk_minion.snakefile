...
rule fqchk_r1_sequences:
    ...
    output:
        os.path.join(RBADIR, "{sample}", "statistics", "{sample}.fqchk.tsv")
    threads: 8
    resources:
        mem_mb=25000
    params:
...
rule get_fqchk_cols:
    input:
        os.path.join(RBADIR, "{sample}", "statistics", "{sample}.fqchk.tsv")
    output:
        temporary(os.path.join(RBADIR, "{sample}", "statistics", "{sample}.fqchk.tsv.tmp"))
    params:
        s = "{sample}"
    shell:
        """
...
rule join_fqchk_cols:
    input:
        expand(os.path.join(RBADIR, "{smp}", "statistics", "{smp}.fqchk.tsv.tmp"), smp=SAMPLES)
    output:
