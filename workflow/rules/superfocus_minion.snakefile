...
rule run_superfocus:
    ...
    output:
        ...
        m81 = os.path.join(RBADIR, "{sample}", "superfocus", f"{FASTQ}_alignments.m8"),
    threads: 16
    params: 
        TMPDIR=TMPDIR
    resources:
    ...
    shell:
        """
        superfocus -q {input.r1} -dir {output.d} -a mmseqs2 -t {threads} -n 0 -tmp $(mktemp -d -p {params.TMPDIR})
        """
...
rule superfocus_taxonomy:
    input:
        m8 = os.path.join(RBADIR, "{sample}", "superfocus", f"{FASTQ}_alignments.m8"),
    output:
        temporary(os.path.join(RBADIR, "{sample}", "superfocus", "{sample}.taxonomy"))
    params:
        t = TAXON
    threads: 4
    resources:
...
rule zip_compress_superfocus:
    input:
        ...
        os.path.join(RBADIR, "{sample}", "superfocus", "{sample}.taxonomy"),
    output:
